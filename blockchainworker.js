export default {
  async fetch(request, env, ctx) {
    ...
    // ==========================================
    // 创世时间戳 (Epoch Start) 与 创世种子节点
    // ==========================================
    const EPOCH_START = 1780108800000;
    const SEED_NODE = 'https://tanzhen.kejikkk.com'; // 你的主站作为全网基石节点
    ...
    if (url.pathname.startsWith('/api/consensus/')) {
        ...
        if (request.method === 'POST' && route === 'submit') {
            // 修复点 1：如果你自己就是创世种子节点，强制无视后台开关接收区块，避免全网断流
    ...
    const mineAndGossip = async (localAsset, localVpsCount) => {
        try {
            // 修复点 3：限制出块高度不低于 1
            const elapsed = Math.max(0, Date.now() - EPOCH_START);
            const currentSlot = Math.max(1, Math.floor(elapsed / 3000)); 
...
`;
    ...
    if (request.method === 'GET' && url.pathname === '/') {
      ...
      try {
          // 修复点 5：正确提取并解析所有人资产，确保累加和排名算法的鲁棒性
          ...
          // 修复点 6：确保兼容多种 true 的字符串和布尔存储格式
          const { results: beacons } = await env.DB.prepare('SELECT count(*) as c FROM blockchain_peers WHERE is_beacon IN ("true", "1")').first();
          activeBeacons = beacons ? beacons.c : 0;
      } catch(e) {
          console.error("Web3 Sync Error:", e);
      }
      ...
      if (isAjax) {
          const ajaxResponse = `
             <div id="ajax-stats-payload" data-rank="${localRank}" data-net-asset="${globalNetAsset.toFixed(2)}" data-proposer="${globalProposer}" data-height="${currentHeight}" data-beacons="${activeBeacons}"></div>
      ...
      <body class="${sys.theme || 'theme1'}">
        <div class="container" id="app-container">
          ...
          <div class="consensus-panel" id="web3-dashboard">
            ...
            <div><div class="c-label">信标权重节点</div><div class="c-val"><span id="ui-beacons">${activeBeacons}</span> 个活跃</div></div>
          </div>
